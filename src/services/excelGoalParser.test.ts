...
describe('extractGoalsFromWorkbook', () => {
  it('extracts 5x 5Km goal from sheet text', () => {
    const buffer = workbookToBuffer(
      [['Objectivos: 5x 5Km ✅', '', ''], ['', '', '']],
      'Plano 2026',
    )

    const goals = extractGoalsFromWorkbook(buffer)
  ...
  it('deduplicates repeated goal mentions in same sheet', () => {
    const buffer = workbookToBuffer(
      [
        ['Objectivos: 5x 5Km', ''],
        ['Meta: 5x 5Km', ''],
