...
As duas têm saída pela porta da frente. O MaxFunSports publica a classificação completa num PDF oficial: descarrega-o no teu browser. No parkrun, abre a página de resultados e imprime-a para PDF. Depois abre o ficheiro na app, no editor do resultado. É lido no teu equipamento e não é enviado para lado nenhum, e o tempo entra marcado como verificado.
...
Both leave the front door open. MaxFunSports publishes the whole ranking as an official PDF, which you download in your browser. For parkrun, open the results page and print it to PDF. Then open the file in the app, in the result editor. It is read on your device and is never sent anywhere, and the time comes in marked as verified.
...
Las dos dejan la puerta principal abierta. MaxFunSports publica la clasificación completa en un PDF oficial: descárgalo en tu navegador. En parkrun, abre la página de resultados e imprímela a PDF. Después abre el archivo en la app, en el editor del resultado. Se lee en tu dispositivo y no se envía a ninguna parte, y el tiempo entra marcado como verificado.
...
Beide lassen die Vordertür offen. MaxFunSports veröffentlicht die vollständige Wertung als offizielles PDF, das du im Browser herunterlädst. Bei parkrun öffnest du die Ergebnisseite und druckst sie als PDF. Dann öffnest du die Datei in der App, im Ergebnis-Editor. Sie wird auf deinem Gerät gelesen und nirgendwohin gesendet, und die Zeit gilt als verifiziert.
...
Les deux laissent la porte d'entrée ouverte. MaxFunSports publie le classement complet dans un PDF officiel, que tu télécharges depuis ton navigateur. Pour parkrun, ouvre la page des résultats et imprime-la en PDF. Ouvre ensuite le fichier dans l'application, dans l'éditeur de résultat. Il est lu sur ton appareil et n'est envoyé nulle part, et le temps est marqué comme vérifié.
...
يمكن لـ Queima Asfalto **البحث عن أوقات الوصول والتصنيفات** في مواقع التوقيت التابعة لجهات خارجية (منظمو السباقات، منصات النتائج، Parkrun، وغيرها) عندما تقدّم رابطًا عامًا للنتائج أو معرّفًا متوافقًا وتطلب إجراء بحث.

كل بحث **يبدأ بمبادرة منك** وينفّذ طلبات HTTP آلية من المثيل (عادةً عبر دالة السحابة `lookupOfficialResults`) على صفحات أو واجهات برمجة تطبيقات **متاحة للعموم**.

#### لا توجد أي علاقة تبعية

نحن غير تابعين لأي منظم سباقات أو مزوّد خدمات توقيت، ولا نحظى بتأييدهم، ولسنا ممثلين لهم (بما في ذلك، من بين آخرين، Parkrun وmika:timing وSporthive وMyRaceResult وEQ Timing وDavengo والمنصات المشابهة). أسماء الخدمات والعلامات التجارية مملوكة لأصحابها المعنيين.

#### شروط خدمة الجهات الخارجية

لكل موقع توقيت **شروط استخدام خاصة به**، وسياسات خصوصية، وقواعد بشأن الوصول الآلي أو نسخ البيانات أو الاستخدام التجاري للنتائج. **قد تكون أنت المسؤول** عن ضمان امتثال استخدامك لهذه الميزة لتلك الشروط.

هذا التنبيه **ليس استشارة قانونية**. في حال الشك، تواصل مع مشغّل المثيل الخاص بك أو مع مستشار قانوني مؤهل.
...
كلاهما يترك الباب الأمامي مفتوحًا. ينشر MaxFunSports الترتيب الكامل في ملف PDF رسمي تُنزّله من متصفحك. أما في parkrun، فافتح صفحة النتائج واطبعها كملف PDF. ثم افتح الملف في التطبيق، في محرر النتيجة. يُقرأ على جهازك ولا يُرسَل إلى أي مكان، ويدخل الزمن موسومًا بأنه مُتحقَّق منه.
...
إذا قمت بنشر مثيلك الخاص، فأنت المسؤول عن كيفية استخدام مستخدميك لهذه الميزة. توصيات:

- أعلِم المستخدمين (مثلًا عبر هذا التنبيه أو صياغة مكافئة في سياسة الخصوصية الخاصة بك).
- راجع شروط مواقع التوقيت التي يستورد منها مستخدموك بشكل متكرر.
- اضبط الحدود في [`docs/cloud-functions-limits.md`](https://github.com/Seven-Panda-Labs/queima-asfalto/blob/main/docs/cloud-functions-limits.md) عند الحاجة.
- فكّر في تعطيل الميزة أو تقييدها إذا لم تستطع الالتزام بالشروط المعمول بها.

الوثائق الكاملة للمشغّلين: [`docs/timing-scraping-disclaimer.md`](https://github.com/Seven-Panda-Labs/queima-asfalto/blob/main/docs/timing-scraping-disclaimer.md).

#### البرمجية

Queima Asfalto برمجية مفتوحة المصدر (AGPL-3.0)، تُقدَّم **«كما هي»**. لا يضمن مؤلفو الشيفرة استمرار توفر الموصلات، ولا يتحملون المسؤولية عن انتهاكات شروط الجهات الخارجية الناشئة عن استخدام هذه الميزة.
